...
import {validator} from '~common/index'
import 'webix/webix.css'
import * as webix from 'webix';
import {$$, ui} from 'webix';

function root() {
    console.log(validator.isEmpty('2'))
...
function saveItem() {
    var form = ($$("film_form") as ui.form);
    var list = ($$("film_list") as ui.list);
...
function deleteItem() {
    var list = $$("film_list") as ui.list;
    var item_id = list.getSelectedId(false);
    if (item_id) {
...
function clearForm() {
    ($$("film_form") as ui.form).clear();
    ($$("film_list") as ui.list).unselectAll();
}

function valuesToForm(id: number) {
    const values = ($$("film_list") as ui.list).getItem(id);
    ($$("film_form") as ui.form).setValues(values)
}
